...
const MatchList = React.memo(props => {
  ...
  }, [selected, loading, setSelected, setRound, data])
  ...
  useEffect(() => {
    ...
    const roundIdx = (data.matches || []).findIndex(
      match => match.id === matchesByRound[round]
    )
        ...
        <React.Fragment>
          <Grid item xs={1}>
            <FormControl>
              ...
              <Select value={round} onChange={onRoundChange}>
                {roundItems}
              </Select>
            </FormControl>
          </Grid>
