...
const MatchHeader = React.memo(() => {
  ...
  const {
    data: { match },
    loading
  } = useQuery(MATCH_INFO, { variables: { id: id } })

  const classes = useStyles({
    homeNumberColor: hex2rgba(
      _.get(match, 'home_team_info.number_color', ''),
      0.4
    ),
    homeShirtColor: hex2rgba(
      _.get(match, 'home_team_info.shirt_color', ''),
      0.4
    ),
...
const Logo = React.memo(({ team, onClick = () => {} }) => {
  ...
  // TODO: add an image container with max size to reserve space
  return team ? (
    <div onClick={onClick}>
      <img src={team.logo} className={classes.img} alt={team.name} />
    </div>
  ) : null
})

const hex2rgba = (hex, alpha = 1) => {
  const [r, g, b] = (mapColor(hex).match(/\w\w/g) || []).map(x =>
    parseInt(x, 16)
  )
...
const ScoreColumn = React.memo(({ team, match }) => {
  ...
  const goals = (match.goals || []).filter(goal =>
    goal.type === 1 ? goal.team_id === teamId : goal.team_id !== teamId
  )
