...
const MatchLineups = React.memo(() => {
  ...
  const { data, error, loading } = useQuery(MATCH_LINEUPS, {
    variables: { id: id }
  })
  ...
  return (
    <Paper elevation={2}>
      <Grid container>
        <LineUp
          data={matchPlayers.filter(mp => mp.team_id === mp.match.home_team_id)}
        />
        <LineUp
          data={matchPlayers.filter(mp => mp.team_id === mp.match.away_team_id)}
        />
      </Grid>
    </Paper>
  )
...
const LineUp = React.memo(({ team, data }) => {
  const hoverId = useSelector(state => state.player.hoverId)
        ...
        <React.Fragment key={`lineup-${entry.player.id}`}>
          ...
          <Grid container>
            ...
            <Grid item xs={8} container justify="flex-start">
              <Player
                data={entry.player}
                hover={entry.player.id === hoverId}
                onMouseEnter={() => setHoverPlayer(entry.player.id)}
                onMouseLeave={() => setHoverPlayer(null)}
                onClick={() => console.log(entry.player)}
              />
              <Goals data={entry.goals} />
              <Cards
                data={entry.cards}
                onClick={() => console.log('card', entry.player)}
              />
            </Grid>
            <Grid item xs={2} container justify="flex-end">
              <Substitutions
                subIn={entry.substitution_in}
                subOut={entry.substitution_out}
              />
            </Grid>
          </Grid>
        </React.Fragment>
    ...
    <React.Fragment>
      {subIn.length > 0 ? (
        <Typography variant="subtitle2">
          {convertHalfSecToMinuteString(subIn[0].second, subIn[0].half)}
          <KeyboardArrowUp fontSize="inherit" />
        </Typography>
      ) : null}
      {subOut.length > 0 ? (
        <Typography variant="subtitle2">
          {convertHalfSecToMinuteString(subOut[0].second, subOut[0].half)}
...
const Goal = React.memo(({ type, standard, time }) => {
  ...
  return (
    <React.Fragment>
      <Typography variant="subtitle2" className={classes.goal}>
        &nbsp;
