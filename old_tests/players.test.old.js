...
describe('Update service: players', () => {
  ...
  describe('updatePlayers', () => {
    ...
    it('updates player with force option', async () => {
      ...
      findOneStub
        .withArgs({ _id: 1 })
        .returns(Promise.resolve(expected[0]))
      findOneAndUpdateStub
        .withArgs({ _id: 1 })
        .returns({ exec: () => Promise.resolve(expectedChanged) })

      const updatedPlayers = await updatePlayers([playerChange], { force: true })

      expect(findOneStub).to.have.been.calledOnce
      expect(findOneAndUpdateStub).to.have.been.calledOnce

      expect(updatedPlayers).eql([expectedChanged])
    })
  ...
  describe('updatePlayersFromDetailedEvent', async () => {
    ...
    it('returns empty with no events or not found events', async () => {
      ...
      expect(await updatePlayersFromDetailedEvent(1, [])).to.eql([])
      expect(await updatePlayersFromDetailedEvent(1, [dummyEvent])).to.eql([])
    })
